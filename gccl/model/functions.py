...
def batch_graphify_multimodal(uni_features, lengths, speaker_tensor, wp, wf, edge_type_to_idx, device):
    node_features, edge_index, edge_type = [], [], []
    batch_size = uni_features.size(0)
    length_sum = 0
    edge_index_lengths = []
    ...
    for j in range(batch_size):
        cur_len = lengths[j].item()
        node_features.append(uni_features[j, :cur_len, :])
        perms = edge_perms(cur_len, wp, wf)
        perms_rec = [(item[0] + length_sum, item[1] + length_sum) for item in perms]
        perms_rec_cross = [(item_rec[0], item_rec[1] + bsz_length_total) for item_rec in perms_rec]
        length_sum += cur_len
        edge_index_lengths.append(len(perms)*2)
        for item, item_rec in zip(perms, perms_rec):
            edge_index.append(torch.tensor([item_rec[0], item_rec[1]]))
            ...
            if item[0] < item[1]:
                ...
                c = "1"
            edge_type.append(edge_type_to_idx["0" + s + c])

        for item, item_rec_cross in zip(perms, perms_rec_cross):
            edge_index.append(torch.tensor([item_rec_cross[0], item_rec_cross[1]]))
            ...
            if item[0] < item[1]:
                ...
                c = "1"
            edge_type.append(edge_type_to_idx["1" + s + c])
